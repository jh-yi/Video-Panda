...
def split_to_even_chunks(indices, lengths, num_chunks):
    ...
    num_indices_per_chunk = len(indices) // num_chunks      ## bsz_per_gpu

    chunks = [[] for _ in range(num_chunks)]                ## #gpus
...
def get_modality_length_grouped_indices(lengths, batch_size, world_size, generator=None):
    ...
    mm_indices, mm_lengths = zip(*[(i, l)
                                 for i, l in enumerate(lengths) if l > 0])  # image or video     -> new: image or text
    lang_indices, lang_lengths = zip(
        *[(i, -l) for i, l in enumerate(lengths) if l < 0]) # text only                          -> new: video
    ...
    megabatch_size = world_size * batch_size                                ## global batch size
    ...
    # if len(additional_batch) > 0:   ## ignore in case of imbalanced gpu load due to video + image samples
...
def get_length_grouped_indices(lengths, batch_size, world_size, generator=None, merge=True):
    ...
    megabatch_size = world_size * batch_size                      ## global batch size
    megabatches = [indices[i: i + megabatch_size].tolist()        ## randomly divide into groups, each group with size = global_bsz
                   for i in range(0, len(lengths), megabatch_size)]      
    megabatches = [sorted(megabatch, key=lambda i: lengths[i],    ## sort (large to small) within group
                          reverse=True) for megabatch in megabatches]
    megabatches = [split_to_even_chunks(
        megabatch, lengths, world_size) for megabatch in megabatches] ## #world_size chunks, each with bsz
...
class LengthGroupedSampler(Sampler):
    ...
    def __init__(
        ...
        self.lengths = lengths              ## list of int
...
class VideoPandaTrainer(Trainer):
    ...
    def _get_train_sampler(self) -> Optional[torch.utils.data.Sampler]:
        ...
        if self.args.group_by_modality_length:
            lengths = self.train_dataset.modality_lengths       # list of int
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.mm_projector_lr is not None and self.args.mm_projector_lr != 0:
                projector_parameters = [name for name, _ in opt_model.named_parameters() if ("mm_projector" in name) or ("video_projector" in name)]
                if self.args.vision_tower_lr is not None and self.args.vision_tower_lr != 0:
                    vision_tower_parameters = [
                        name for name, _ in opt_model.named_parameters() if ("vision_tower" in name) or ("video_tower" in name)]
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False): # TODO: not supported yet
            ...
            super(VideoPandaTrainer, self)._save_checkpoint(model, trial, metrics)

    def _save(self, output_dir: Optional[str] = None, state_dict=None):
        if getattr(self.args, 'tune_mm_mlp_adapter', False):
            pass
        else:
            super(VideoPandaTrainer, self)._save(output_dir, state_dict)
