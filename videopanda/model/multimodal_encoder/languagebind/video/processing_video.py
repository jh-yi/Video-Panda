...
def load_and_transform_video(
    ...
    if video_decode_backend == 'pytorchvideo':
        ...
        video_data = decord_vr.get_batch(frame_id_list) ## np -> tensor
...
class LanguageBindVideoProcessor(ProcessorMixin):
    ...
    def __init__(self, config, tokenizer=None, **kwargs):
        ...
        if isinstance(config, dict) and config.get('patch_stride_teacher'):
            ...
            self.all_num_frames = config['all_num_frames']
            self.image_mean = np.array(OPENAI_DATASET_MEAN).reshape(1,1,3) # compatible with InternVideoV2
            ...
            [
                ...
                Lambda(lambda x: x / 255.0),
                NormalizeVideo(mean=OPENAI_DATASET_MEAN, std=OPENAI_DATASET_STD),
                ShortSideScale(size=self.image_size),
                CenterCropVideo(self.image_size),
                RandomHorizontalFlipVideo(p=0.5),])
        else:
            self.image_size = config.image_size
            self.patch_stride = config.patch_stride
    ...
    def __call__(self, images=None, text=None, context_length=77, return_tensors=None, **kwargs):
        ...
        if images is not None:
            ...
            image_features = [self.transform(image) for image in images] # images: list of tensor (C, T, H, W), but only 1 item 
